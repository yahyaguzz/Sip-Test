...
const Content: React.FC = () => {
    ...
    return (
        <div className="flex flex-1 flex-col gap-2">
            <div>
                {sessions.map((session, index) => <div key={`session-${index}-${session.number}`} className="flex flex-row bg-gray-300">
        ...
        /> */}
                <button className="bg-sky-50" onClick={handleRegister}>Register</button>
                <button className="bg-sky-50" onClick={unRegister}>unRegister</button>
                {/* Arama Yapma */}
                {registerer && (
                    <>
                        <input
                        ...
                        <button className="bg-sky-50" onClick={handleCall}>Arama Yap</button>
                        {currentSession &&
                            <>
                                <button className="bg-sky-50" onClick={async () => await terminate()}>Kapat</button>
                                <button className="bg-sky-50" onClick={async () => await toggleHold()}>{currentSession?.sessionState === CustomSessionState.Held ? "Beklemede" : "Beklemeye Al"}</button>
                            </>}
                        <button className="bg-sky-50" onClick={toggleMute}>{isMute ? "Mikrofonu Aç" : "Mikrofonu Kapat"}</button>
            ...
            <div>
                ...
                <div className="flex flex-col w-fit items-center justify-center bg-sky-50 p-2">
                    {/* Giriş Alanı */}
                    ...
                    <div className="grid grid-cols-3 gap-4 w-40">
                        {keys.map((key) => (
                            <button
                                ...
                                className="bg-slate-500 text-white font-bold py-2 px-4 rounded hover:bg-slate-700 active:bg-slate-800"
                            >
                                {key}
                            </button>
                        ))}
